...
$checks = ,@('res://tools/build_architecture_lab.gd', 'arch_lab_build.log', '600', '^ARCH_LAB_BUILD: \d+ brushes; save=0$')
if ($Validate) {
    $checks += ,@('res://tools/validate_architecture_lab.gd', 'arch_lab_qa.log', '20000', '^ARCH_LAB_QA: \d+ checks; 0 failures$')
}
if ($Capture) {
    $checks += ,@('res://tools/capture_architecture_lab.gd', 'arch_lab_capture.log', '4000', '^ARCH_LAB_CAPTURE: PASS$')
}
