...
func run() -> void:
	var resource: PackedScene = load("res://architecture/architecture_lab.tscn")
	if resource == null:
		push_error("ARCH_LAB_BUILD_FAILED: scene missing")
		quit(1)
		return
	...
	if not result.complete or result.failed or geometry.get_child_count() == 0:
		push_error("ARCH_LAB_BUILD_FAILED")
		quit(1)
		return
	for child in geometry.find_children("*", "", true, false): child.owner = scene
	var packed := PackedScene.new()
	var error := packed.pack(scene)
	if error == OK: error = ResourceSaver.save(packed, "res://architecture/architecture_lab.tscn")
	print("ARCH_LAB_BUILD: ", geometry.find_children("*", "CollisionShape3D", true, false).size(), " brushes; save=", error)
